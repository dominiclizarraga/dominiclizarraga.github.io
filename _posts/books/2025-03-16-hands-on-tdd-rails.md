...
This is the central idea of my book. We will think about what we want to do, then we will capture that idea with a properly failing test. Then we will then write the code to make the test pass. After that, we will then we refactor our code to make it better, or prettier, or less repetitive.
...
We will use `Rspec` testing framework to write our test. `Rspec` describes itself as a tool for performing behavioral driven development and as in fact the best tool available for testing will be code.
...
The author states the following, <b>I have never seen software written using TDD that was not better than software written without TDD.</b>
