...
10. Finally, we noticed that we had repeating conditionals in that `BottleNumber` class, and we cured those conditionals by following the `Replace Conditional` with `Polymorphism` recipe and making a little inheritance hierarchy of different players of the `BottleNumber` role.
