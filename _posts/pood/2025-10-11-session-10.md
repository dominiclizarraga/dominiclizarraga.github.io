...
In the last section we implemented a little inheritance hierarchy that once we wanted to introduce a new change it flopped.
...
In object-oriented programming, to expect that if someone knows enough to pass you a `true` or a `false`, or a `truck` or a `carrot`, they know enough to pass you a smarter thing.
...
# we can now invoke RandomOrderer and PiratePrefixer
puts House.new(orderer: RandomOrderer.new).line(12)
puts
puts House.new(prefixer: PiratePrefixer.new).line(12)
```

We can name them after we have written down the code, remember `Object` are, not doers.
