...
WOW! it look quite the same!! I was about to start interpolating like the last time since i notice the pattern in each sentence but I resisted to make the premature abstraction !
...
We'll ignore conditional, and stey with magic number and code duplication.

"Which one of those is more closely related to the thing I'm trying to vary?", and it's the duplication, so let's work on that.

We have a recipe that lets us remove duplication -- that is, it allows us to turn difference into sameness and that's the Flocking Rules.
...
This one seems clever but if we add another branch for number 3 we'll end up with same conditionals like original code, remember we are looking for an algorithm that all different variants and doesn't contain any duplication and that will enable to treat everything the same.

Using some patter matching skills we discovered that the slots of the phrases we are pluging in have more meaning that the string itself, they have the position in which they are going to be placed, therefore we can use to design the algorithm that handle varieties.
